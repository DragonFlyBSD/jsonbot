...
def subscribe(url):
    ...
    if credentials.HUB_CREDENTIALS:
        auth_string = "Basic " + base64.b64encode(
            "%s:%s" % tuple(credentials.HUB_CREDENTIALS)
        )
...
class HubbubItem(Persist):
    ...
    def fetchdata(self):
        ...
        if result and "bozo_exception" in result:
            logging.info(
                "hubbub - %s bozo_exception: %s" % (url, result["bozo_exception"])
            )
        try:
...
class HubbubWatcher(PlugPersist):
    ...
    def work(self, botname, type, channel, entries, url, *args, **kwargs):
        try:
            ...
            if not res2:
                logging.info(
                    "no updates for %s (%s) feed available" % (item.data.name, channel)
                )
                return
                ...
                if len(i) == 3:
                    try:
                        ...
                        try:
                            ...
                            logging.info(
                                "hubbub - %s is not in the format (botname, bottype, channel)"
                                % str(item)
                            )
                            continue
                else:
    ...
    def makeresponse(self, name, res, channel, sep=" .. "):
        ...
        for j in res:
            if (
                item.markup.get(jsonstring([name, channel]), "skipmerge")
    ...
    def start(self, botname, type, name, channel):
        ...
        target = channel
        if (
            not jsonstring([botname, type, target]) in item.data.watchchannels
            and not [botname, type, target] in item.data.watchchannels
        ):
    ...
    def clone(self, botname, type, newchannel, oldchannel):
        ...
        logging.debug(
            "hubbub - clone - %s - %s - %s - %s - %s"
            % (botname, type, newchannel, oldchannel, feeds)
        )
        for feed in feeds:
...
def handle_hubbubsubscribe(bot, event):
    ...
    for name in event.args:
        ...
        if name not in watcher.data["urls"]:
...
examples.add(
    "hb-add",
    "hubbub-add <name> <url> to the watcher",
...
examples.add(
    "hb-watch",
    "hubbub-watch <name> [seconds to sleep] .. go watching <name>",
    "hb-watch gozerbot",
)
        ...
        if wave and wave.data:
            ...
            if wave.data.loud:
                try:
                    ievent.set_title(
                        "JSONBOT - %s - #%s"
                        % (" - ".join(wave.data.feeds), str(wave.data.nrcloned))
                    )
                except Exception as ex:
                    handle_exception()
    if started:
...
examples.add(
    "hb-start",
    "hubbub-start <list of feeds> .. start a hubbub feed (per user/channel) ",
    "hb-start gozerbot",
)
...
def handle_hubbubstop(bot, ievent):
    ...
    cantstop = []
    if feeds[0] == "all":
        feeds = watcher.listfeeds(bot.cfg.name, bot.type, ievent.channel)
    for name in feeds:
...
examples.add(
    "hb-stop",
    "hubbub-stop <list of names> .. stop a hubbub feed (per user/channel) ",
    "hb-stop gozerbot",
)
...
examples.add(
    "hb-stopall",
    "hubbub-stopall .. stop all hubbub feeds (per user/channel) ",
    "hb-stopall",
)
...
def handle_hubbubaddchannel(bot, ievent):
    ...
    if not item.data.running:
        ...
        return
    if (
        jsonstring([botname, type, channel]) in item.data.watchchannels
        or [botname, type, channel] in item.data.watchchannels
    ):
...
examples.add(
    "hb-addchannel",
    "add a channel to a feeds watchlist",
...
examples.add(
    "hubbub-delchannel",
    "remove a channel from a feeds watchlist",
...
examples.add(
    "hb-stopwatch",
    "hubbub-stopwatch <name> .. stop polling <name>",
...
examples.add(
    "hb-itemslist",
    "hb-itemslist <name> .. get itemslist of <name> ",
...
examples.add(
    "hb-feeds",
    "hb-feeds <name> .. show what feeds are running in a channel",
    "1) hb-feeds 2) hb-feeds #dunkbots",
)
...
def handle_hubbubregister(bot, ievent):
        ...
        if item:
            if not name in watcher.getfeeds(ievent.channel, bot.cfg.name):
                ...
                if name not in ievent.chan.data.feeds:
                    ...
                    ievent.chan.save()
                ievent.reply(
                    "started %s feed. entries will show up when the feed is updated."
                    % name
                )
            else:
    ...
    if int(result.status) > 200 and int(result.status) < 300:
        if watcher.add(name, url, ievent.userhost):
            watcher.start(bot.cfg.name, bot.type, name, target)
            ievent.reply(
                "started %s feed. entries will show up when the feed is updated." % name
            )
            return
        else:
            ievent.reply(
                "there already exists a %s feed. please choose a different name" % name
            )
            return
    else:
        ievent.reply(
            "feed %s NOT added. Status code is %s. please check if the feed is valid."
            % (name, result.status)
        )
...
examples.add(
    "hb-register",
    "hb-register .. register url and start it in one pass",
    "hb-register hgrepo http://code.google.com/feeds/p/jsb/hgchanges/basic",
)
